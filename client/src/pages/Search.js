		...
		<CContainer>
			...
			{search ? (
				<div style={{ margin: "1rem" }}>
					...
					>
						{filteredList(users, category, search).map((user, id) => (
							<Grid item xs={3} key={user.id}>
								<TutorCard name={user.name} category={user.category} image={`https://picsum.photos/id/${id+1}/200/300`} phone={user.phone} />
							</Grid>
						))}
					</Grid>
				</div>
